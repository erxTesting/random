/*
 apache access log GSI Trim import to MySQL
*/
...
LOAD DATA INFILE '/tmp/gsitrim.log' INTO TABLE apachelog
  FIELDS TERMINATED BY '"' OPTIONALLY ENCLOSED BY '' ESCAPED BY ''
  (@var1, vhost,  remote_host, last_request_status, request_method, request_path, 
  query_string, bytes_sent, cookie, session, user_agent, referrer, 
  response_microseconds)

SET date_time = STR_TO_DATE( @var1, '%H:%i:%s');
